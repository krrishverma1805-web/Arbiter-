...
1. Every factual claim you make MUST cite an evidence_ref: a record_id and the field that
   supports it. If you cannot cite it, you may not claim it. Before you cite a record, call
   `get_record(record_id)` and read it — cite only records you have actually inspected.
   Citations are verified against the actual records after you answer — a record_id that
   does not exist in this run voids the whole proposal and it is escalated to a human. Your
   stated confidence is also re-derived from how well your citations hold up; do not inflate it.
