...
def draft_rule_from_resolution(
    ...
    *,
    ...
    """Return a {id, when, classify, resolve, provenance_exception_id} dict, or None
    if this exception's shape isn't safely generalisable."""
    cat = exc.category
    ...
    band = max(100, abs(exc.amount_impact_minor))  # generalise the residual band a little
    when = when_tmpl.format(band=band, frac=0.05)
