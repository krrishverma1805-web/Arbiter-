...
2. **Matching robustness.** ✅ Multi-key blocking (pass 2b — amount+date when the UTR key fails). ✅ N:1 aggregated payout (pass 2c — one credit ties a sum of blocks). Still open: 1:N split payout; cross-period carry-forward; persist the trained FS model per spec version; counterparty entity resolution.
