...
18. Apple-minimal design system pass; realtime presence + live scorecard over WebSocket. — ✅ **`WS /v1/runs/{id}/ws` + `arbiter_api/presence.py`: an in-process room hub — every open cockpit on a run sees the others' initials + "N viewing" in the header, and a resolve is fanned out so two analysts stay in sync without polling (`usePresence` degrades silently to solo if the socket won't connect). Swap `_Hub` for Redis pub/sub for multi-replica.** Still open: live scorecard streaming into the cockpit (the `live` view already streams counts).
