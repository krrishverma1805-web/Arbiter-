...
11. OpenTelemetry + structlog + Prometheus/Grafana + Sentry + SLOs + alerting. — ✅ **structlog JSON logs + request-id correlation (`X-Request-Id`) + `GET /metrics` (Prometheus: request counts/latency, run outcomes, queue depth).** ✅ **OpenTelemetry span export (`arbiter_engine/tracing.py` no-op shim + `arbiter_api.obs.configure_tracing` — OTLP/HTTP exporter, FastAPI + SQLAlchemy instrumentation, engine `run → ingest → match → classify → investigate` spans; opt-in via `OTEL_EXPORTER_OTLP_ENDPOINT` + the `[observability]` extra).** ✅ **Sentry (`configure_sentry`, opt-in via `SENTRY_DSN`).** Still open: Grafana dashboards, SLO alert rules.
12. Redis cache + CDN + autoscaling + pgbouncer; backup + **restore drill in CI**; chaos test. — ✅ **CI `recovery` job: a real Postgres, `arbiter-api db upgrade` (exercises the RLS migration for real), a deterministic run, `pg_dump -Fc` → `DROP SCHEMA` → `pg_restore`, then `arbiter verify --json` + `replay` assert the terminal hash, event count and chain integrity are byte-identical after the round trip.** ✅ **Chaos: a killed worker self-heals — `jobs.reclaim_stale()` (run at the top of every `claim()`) requeues a `running` job whose lease (`ARBITER_JOB_LEASE_SECONDS`, default 300s) has expired, or dead-letters it if out of attempts; two tests cover both.** ✅ **HPA on api/worker/web + `autoscaling/v2` in the Helm chart.** Still open: Redis cache layer, a CDN in front of the cockpit, pgbouncer sidecar.
