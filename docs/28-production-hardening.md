...
2. **Matching robustness.** ✅ Multi-key blocking (pass 2b — amount+date when the UTR key fails). ✅ N:1 aggregated payout (2c) and 1:N split payout (2d). Still open: cross-period carry-forward (needs a cross-run match model); persist the trained FS model per spec version; counterparty entity resolution.
