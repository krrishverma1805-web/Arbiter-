...
3. **Agent accuracy.** Grounding enforcement (reject proposals whose `evidence_refs` don't resolve); verifier pass; RAG resolution memory (pgvector); wire tiered triage; agent-confidence calibration; self-consistency for high-$ exceptions.
