...
8. Object storage for uploads; rate limiting; idempotency keys; published OpenAPI. — ✅ **per-tenant rate limiting (`ratelimit.py`)**; ✅ **tenant-scoped upload storage (`POST /v1/uploads` → `dataset: "upload:<id>"`, `storage.py` behind a swappable interface)**. Still open: an S3/R2 storage backend, idempotency keys, a pinned OpenAPI snapshot test.
