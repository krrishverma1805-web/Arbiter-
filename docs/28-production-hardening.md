...
8. Object storage for uploads; rate limiting; idempotency keys; published OpenAPI. — ✅ **per-tenant rate limiting (`ratelimit.py`)**; ✅ **tenant-scoped upload storage (`POST /v1/uploads` → `dataset: "upload:<id>"`, `storage.py` behind a swappable interface)**. ✅ **idempotency keys (`idempotency.py`, `Idempotency-Key` header, 409 on reuse-with-a-different-body)**. ✅ **a pinned OpenAPI-surface snapshot test (`test_openapi.py` + `openapi-surface.json`) — CI fails on any unintended route add/remove/verb change.** Still open: an S3/R2 storage backend (the interface is ready; needs `boto3`).
