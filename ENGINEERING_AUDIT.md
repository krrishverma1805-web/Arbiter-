...
| A2 | G6 — Attack Arbiter harness + CLI + UI button | strongest "Failure Recovery" demo move; "watch Arbiter refuse to be fooled" | ✅ **DONE** — `attack.py` (12 deterministic dataset-mutation scenarios), `arbiter attack` CLI (`--scenario`/`--json`, exits 1 on any UNSAFE), `POST /v1/attack`, cockpit `AttackPanel` ("Run the attack suite" button), `test_attacks.py` + API test. Hardened 4 real gaps found by the harness: injection scanner scope (`injection.py`), foreign-currency quarantine + implausible-date quarantine (`normalize.py`), bank-credit linkage (`classify.py`). Result: **12 contained · 0 partial · 0 missed · 0 UNSAFE**, ₹0 unaccounted |
...
| Product (control room, evidence, exception workflow, agent activity, attack mode, replay, benchmark dash, close memo, arch viz) | ✅ · **attack CLI + `POST /v1/attack` + cockpit panel: A2 DONE** · **root-cause clusters + `GET /clusters` + panel: B1 DONE** |
